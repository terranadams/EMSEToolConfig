/*------------------------------------------------------------------------------------------------------/
| Program : ACA_Onload_Sample_V1.6.js
| Event   : ACA_Before
|
...
try {
    ...
    var c = new contactObj(cap.getApplicantModel());

    if (c) {
        if (String(c.people.getContactTypeFlag()).toUpperCase() == "ORGANIZATION") {
            
            showMessage = true;
            cancel = true;
            comment("Cannot complete a name change for a business, must file a new application.");
        }
    }
